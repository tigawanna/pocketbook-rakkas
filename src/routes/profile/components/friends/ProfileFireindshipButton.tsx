...
import { PocketbookUserResponse, PocketbookFriendshipResponse } from "@/lib/pb/db-types";
import { useUser } from "@/lib/rakkas/hooks/useUser";
import { createFriendship, followUnfollowOrFollowBack, shouldFollowUnfollowOrFollowBack } from "@/state/models/friends/frenship";
...
export function ProfileFireindshipButton({profile_user}:profileFireindshipButtonProps){
const {user,pb} =useUser()
const query = useQuery({
  queryKey: ["profile","friendship", profile_user.id],
  queryFn: () => tryCatchWrapper(
      pb.collection("pocketbook_friendship").getFirstListItem(
        or(
          and(["user_a.id", "=", user.id], ["user_b.id", "=", profile_user.id]),
          and(["user_b.id", "=", user.id], ["user_a.id", "=", profile_user.id]),
        ),
        {},
      ),
    ),
...
const follow_mutation = useMutation({
  mutationFn: (vars) => createFriendship({ 
    pb,
    me: user.id,
...
console.log(" ========= frenship ========= ",{update_friendship_mutation})
if(query.isPending){
    return (
...
const type = shouldFollowUnfollowOrFollowBack({
  frenship: data, logged_in_id: user.id, profile_id: profile_user.id})
